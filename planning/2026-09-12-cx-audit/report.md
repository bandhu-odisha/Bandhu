...
- **W03** — Add `{% block meta %}` to `templates/base.html` and real `<title>`/description/`og:title`/`og:description`/`og:image` (use the hero image) on `landing_react.html` and each program's `list_page.html`; rename the "Ashram" title to "Bandhughar". **Status: Implemented 2026-09-12** — `templates/base.html` (default meta block), `templates/landing_react.html`, `templates/initiative_program/list_page.html` (description/OG/Twitter, with `content`-based fallbacks), `applications/ashram/templates/ashram.html` (title → "Bandhu | Bandhughar").
- **W04** — In `bandhu/settings.py` set `SESSION_COOKIE_SECURE = CSRF_COOKIE_SECURE = not DEBUG`, `SECURE_HSTS_SECONDS = 31536000` (+ `SECURE_HSTS_INCLUDE_SUBDOMAINS`), `SECURE_PROXY_SSL_HEADER` for Cloudflare; ship with the next deploy. **Status: Implemented 2026-09-12.**
- **W05** — Give the dot buttons a 44×44 hit area (padding or `before:` pseudo-element, keep the visual dot small), bump nav/ticker/modal link padding to ≥ 24 px tall. **Status: Implemented 2026-09-12** — dots get a 44px-tall hit area via button padding + a visible-dot inner `<span>`, and row `gap` was widened (not just padding) so center-to-center spacing also clears the WCAG 2.5.8 24px minimum; nav/footer/ticker/modal link padding bumped in `css/bandhu.css`, `css/custom.css`, `css/accounts-auth-landing.css` (+ `static/css/` mirrors). Note: the React landing page's own `Navbar.jsx`/`Footer.jsx` tap targets were out of this pass's file scope and were not audited/touched.
- **W07** — Replace `bandhu-logo-navbar.png` with an SVG or a 210 px WebP (~5 KB). **Status: Implemented 2026-09-12** — replaced with a 210×210 WebP (~5.3 KB, was 343 KB); old PNG removed, all template/view references updated.
- **W09** — Add visually-hidden `<label for>` elements to the 5 modal inputs; fix the `for` ids in the admin modals. **Status: Implemented 2026-09-12** — labels added to `accounts/templates/includes/login_modal.html` and `signup_modal.html`. The admin-modal for/id bug was not on `templates/initiative_program/*` (those were already correct) — the real source, found via a live console warning on `/bandhughar/`, was `applications/ashram/templates/ashram.html`'s legacy "Create Bandhughar" modal (empty `for=""` × 5, one `for="logo"` pointing at a nonexistent id); fixed there. The same pattern still exists, unfixed, in `applications/ashram/templates/ashram_detail.html`, `applications/ankurayan/templates/ankurayan.html`, and `applications/anandakendra/templates/anandakendra.html` — flagged as a follow-up, not part of this pass.
- **W10** — Apply the same caption CSS fix from commit `b09e33c` to `year_detail.html:82` / `css/anandakendra.css:834`. **Status: Implemented 2026-09-12, different root cause than cited.** The real `/bandhughar/detail/<slug>/` URL renders `applications/ashram/templates/ashram_detail.html` (and `/anandakendra/detail/<slug>/` renders `applications/anandakendra/templates/anandkendra_detail.html`) — legacy templates that load `css/anandakendra.css` but never load `initiative-program-year.css`, so commit `b09e33c`'s `body.initiative-year-page` fix was never reachable there. `templates/initiative_program/year_detail.html` (the shared clone-app template) was already correctly contained. Fix: new rules scoped to `body.anandakendra-detail-page` in `css/anandakendra.css`/`static/css/anandakendra.css` (caption max-width/overflow containment + a `≤767px` flex-wrap rule so the caption and "Back to {program}" pill don't compete for one row).
