...
def index(request):
    context = ipy.build_index_context(request, 'patriotism', initiative_models)
    context.update(ipy.build_index_gallery_context(request, 'patriotism', initiative_models))
    return render(request, 'patriotism/patriotism.html', context)
...
def create_ashram(request):
    if request.method == 'POST':
        ...
        if Ashram.objects.filter(name=name, locality=locality).exists():
            ...
            return redirect('patriotism:patriotism')
        ...
        return redirect('patriotism:AshramDetail', slug)

    return redirect('patriotism:patriotism')

def ashram_detail(request, slug):
    return ipy.render_year_detail(request, slug, 'patriotism', initiative_models)


upload_report_file = ipy.make_upload_report_file('patriotism', initiative_models)
add_report_link = ipy.make_add_report_link('patriotism', initiative_models)
delete_report_file = ipy.make_delete_report_file('patriotism', initiative_models)
delete_report_link = ipy.make_delete_report_link('patriotism', initiative_models)
upload_invitation = ipy.make_upload_invitation('patriotism', initiative_models)
delete_invitation = ipy.make_delete_invitation('patriotism', initiative_models)
update_description = ipy.make_update_description('patriotism', initiative_models)
...
def add_attendee(request):
    if request.method == 'POST':
        ...
        url = '/patriotism-in-action/detail/' + slug +'/'
...
add_activity_category = ipy.make_add_activity_category('patriotism', initiative_models)
create_activity = ipy.make_create_activity('patriotism', initiative_models)
add_to_gallery = ipy.make_add_to_gallery('patriotism', initiative_models)

@login_required
def admin_approval(request):
    if request.method == 'POST':
        slug = request.POST.get('patriotism')
        ...
        if request.META.get('HTTP_X_REQUESTED_WITH') != 'XMLHttpRequest':
            url = reverse('patriotism:AshramDetail', kwargs={'slug': slug}) + '#gallery'
            return HttpResponseRedirect(url)
...
create_event = ipy.make_create_event('patriotism', initiative_models)