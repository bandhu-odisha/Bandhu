...
class AshramAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Schedule', {
            ...
            'description': 'Shown below the hero image on the activity detail page, like Ankurayan year schedules.',
        }),
        entry_hero_fieldset('image'),
    )
    list_display = ('name', 'locality', 'start_date', 'end_date')

    def response_add(self, request, obj, post_url_continue=None):
        next_site = request.GET.get('next')
        if next_site == 'sevavrata_details':
            return HttpResponseRedirect(reverse('sevavrata:AshramDetail', args=(obj.slug,)))
