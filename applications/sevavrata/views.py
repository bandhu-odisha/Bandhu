...
def index(request):
    context = ipy.build_index_context(request, 'sevavrata', initiative_models)
    context.update(ipy.build_index_gallery_context(request, 'sevavrata', initiative_models))
    return render(request, 'sevavrata/sevavrata.html', context)
...
def create_ashram(request):
    if request.method == 'POST':
        ...
        if Ashram.objects.filter(name=name, locality=locality).exists():
            ...
            return redirect('sevavrata:sevavrata')
        ...
        return redirect('sevavrata:AshramDetail', slug)

    return redirect('sevavrata:sevavrata')

def ashram_detail(request, slug):
    return ipy.render_year_detail(request, slug, 'sevavrata', initiative_models)


upload_report_file = ipy.make_upload_report_file('sevavrata', initiative_models)
add_report_link = ipy.make_add_report_link('sevavrata', initiative_models)
delete_report_file = ipy.make_delete_report_file('sevavrata', initiative_models)
delete_report_link = ipy.make_delete_report_link('sevavrata', initiative_models)
upload_invitation = ipy.make_upload_invitation('sevavrata', initiative_models)
delete_invitation = ipy.make_delete_invitation('sevavrata', initiative_models)
update_description = ipy.make_update_description('sevavrata', initiative_models)
...
add_activity_category = ipy.make_add_activity_category('sevavrata', initiative_models)
create_activity = ipy.make_create_activity('sevavrata', initiative_models)
add_to_gallery = ipy.make_add_to_gallery('sevavrata', initiative_models)

@login_required
def admin_approval(request):
    if request.method == 'POST':
        slug = request.POST.get('sevavrata')
        ...
        if request.META.get('HTTP_X_REQUESTED_WITH') != 'XMLHttpRequest':
            url = reverse('sevavrata:AshramDetail', kwargs={'slug': slug}) + '#gallery'
            return HttpResponseRedirect(url)
...
create_event = ipy.make_create_event('sevavrata', initiative_models)